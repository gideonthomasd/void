...
class Module(core.module.Module):
    ...
    def _format(self):
        if util.format.asbool(self.parameter("usedonly", False)):
            ...
            return self.parameter("format", "  {used}/{total}")
#({percent:05.02f}%)
    ...
    def update(self):
        ...
        if "Memailable" in data:
            used = data["MemTotal"] - data["MemAvailable"]
            #used-bash = "free -h | awk '(NR==2) {print$3}'"
           # used-bash = subprocess.run('/usr/share/bumblebee-status/bumblebee_status/modules/core/mem.sh')
           #used = json.loads(requests.get(used-bash).text)
        else:
            used = (
                data["MemTotal"]
                - data["MemFree"]
